...
N::HairsView:: HairsView     ( QWidget * parent , Plan * p )
             : ListDock      (           parent ,        p )
             , PeopleManager (                           p )
             , dropAction    ( false                       )
{
...
N::HairsView::~HairsView(void)
{
}

void N::HairsView::Configure(void)
{
  setWindowTitle               ( tr("Hair colors")    ) ;
...
bool N::HairsView::hasItem(void)
{
  QListWidgetItem * item = currentItem() ;
  return NotNull ( item )                ;
}

bool N::HairsView::startDrag(QMouseEvent * event)
{
...
bool N::HairsView::fetchDrag(QMouseEvent * event)
{
  if (!IsMask(event->buttons(),Qt::LeftButton)) return false   ;
...
void N::HairsView::dragDone(Qt::DropAction dropIt,QMimeData * mime)
{
}

bool N::HairsView::finishDrag(QMouseEvent * event)
{
  return true   ;
}

bool N::HairsView::acceptDrop(QWidget * source,const QMimeData * mime)
{
  return dropHandler ( mime ) ;
}

QString N::HairsView::MimeType(const QMimeData * mime)
{
...
UUIDs N::HairsView::MimeUuids(const QMimeData * mime,QString mimetype)
{
  UUIDs      Uuids                       ;
...
bool N::HairsView::dropNew(QWidget * source,const QMimeData * mime,QPoint pos)
{
  QString    mtype                  ;
  UUIDs      Uuids                  ;
...
bool N::HairsView::dropMoving(QWidget * source,const QMimeData * mime,QPoint pos)
{
...
bool N::HairsView::dropAppend(QWidget * source,const QMimeData * mime,QPoint pos)
{
  if (dropAction) return false             ;
  return dropItems ( source , mime , pos ) ;
}

bool N::HairsView::startup(void)
{
  clear ( )                                           ;
  GroupItems     GI ( plan )                          ;
  ...
  EnterSQL ( SC , plan->sql )                         ;
    ...
    UUIDs   Uuids = SC.Uuids                          (
      PlanTable(Hairs) , "uuid" , "order by id asc" ) ;
    foreach (uuid,Uuids)                              {
      QIcon icon                                      ;
      ...
      if (NotNull(image))                             {
        ...
        icon = QIcon(":/images/eyeshadower.png")      ;
      }                                               ;
...
bool N::HairsView::Menu(QPoint pos)
{
  nScopedMenu     ( mm , this )                                      ;
...
bool N::HairsView::dropTags(QWidget * source,QPointF psf,const UUIDs & Uuids)
{
...
bool N::HairsView::dropPictures(QWidget * source,QPointF psf,const UUIDs & Uuids)
{
  if (source==this) return true                   ;
  QPoint pos = psf.toPoint()                      ;
  QListWidgetItem * atItem = itemAt(pos)          ;
...
bool N::HairsView::dropPeople(QWidget * source,QPointF psf,const UUIDs & Uuids)
{
  QPoint pos = psf.toPoint()             ;
  QListWidgetItem * atItem = itemAt(pos) ;
  if (source==this) return true          ;
