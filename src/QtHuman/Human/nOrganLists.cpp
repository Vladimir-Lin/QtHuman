...
N::OrganLists:: OrganLists (QWidget * parent,Plan * p)
              : TreeDock   (          parent,       p)
...
N::OrganLists::~OrganLists(void)
{
}

QSize N::OrganLists::sizeHint(void) const
{
  return QSize ( 240 , 240 ) ;
}

bool N::OrganLists::FocusIn(void)
{
...
bool N::OrganLists::acceptDrop(QWidget * source,const QMimeData * mime)
{
  nKickOut ( nEqual(source,this) , false ) ;
  return dropHandler(mime)                 ;
}

bool N::OrganLists::dropText(QWidget * source,QPointF pos,const QString & text)
{ Q_UNUSED ( source ) ;
  Q_UNUSED ( pos    ) ;
  Paste    ( text   ) ;
  return true         ;
...
void N::OrganLists::Configure(void)
{
  PassDragDrop = false                          ;
  ///////////////////////////////////////////////
  NewTreeWidgetItem       ( head              ) ;
  head -> setText         ( 0 , tr("Organ")   ) ;
  setWindowTitle          ( tr("Organs")      ) ;
...
UUIDs N::OrganLists::LoadUuids(SqlConnection & connection)
{
...
void N::OrganLists::LoadNames(SqlConnection & connection,UUIDs & Uuids)
{
  int  index = 0                      ;
  SUID uuid  = 0                      ;
  QTreeWidgetItem * it                ;
...
SUID N::OrganLists::AppendUuid(SqlConnection & connection)
{
  QString Q                                  ;
  SUID    uuid = 0                           ;
...
bool N::OrganLists::AppendName(SqlConnection & connection,SUID uuid,QString name)
{
  return connection.assureName(PlanTable(Names),uuid,vLanguageId,name) ;
}

QTreeWidgetItem * N::OrganLists::AppendItem (void)
{
...
void N::OrganLists::setCheckable(QTreeWidgetItem * item)
{ Q_UNUSED ( item ) ;
}

bool N::OrganLists::List(void)
{
...
void N::OrganLists::singleClicked(QTreeWidgetItem * item,int column)
{
}

void N::OrganLists::doubleClicked(QTreeWidgetItem * item,int column)
{
  QLineEdit * le = NULL                  ;
  switch (column)                        {
    case 0                               :
...
void N::OrganLists::removeOldItem(void)
{
  TreeWidget :: removeOldItem ( true , 0 ) ;
}

void N::OrganLists::editingFinished(void)
{
...
void N::OrganLists::Paste(QString text)
{
...
void N::OrganLists::Paste(void)
{
  QString text = qApp->clipboard()->text() ;
  Paste(text) ;
}

void N::OrganLists::Export(void)
{
...
void N::OrganLists::Translation(void)
{
  UUIDs Uuids = itemUuids ( 0 )              ;
  emit Translations ( windowTitle() , Uuids) ;
}

bool N::OrganLists::Menu(QPoint pos)
{
  ...
  mm . add ( 105 , tr("Copy"   ) )                                 ;
  nIfSafe(item)                                                    {
    mm . add ( 111 , tr("List %1").arg(item->text(0)) )            ;
  }                                                                ;
  mm . addSeparator (                   )                          ;
  ...
  switch (mm[a])                                                   {
    ...
    case 111                                                       :
      nIfSafe(item)                                                {
        emit BodyParts ( item->text(0) , nTreeUuid(item,0) )       ;
      }                                                            ;
    break                                                          ;
    case 1000001                                                   :
